...
export const UserProfile: React.FC<ComponentProps> = ({
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      // Should not warn about unused error
  ...
  return (
    <div className="user-profile">
      ...
      {isEditing ? (
        ...
        >
          <input type="text" value={name} onChange={handleInputChange('name')} placeholder="Name" />
          <input type="email" value={email || ''} onChange={handleInputChange('email')} placeholder="Email" />
      ...
      <p>
        Second item: {secondItem}, Others: {JSON.stringify(otherItems)}
      </p>
    </div>
  )
}
